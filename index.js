...
async function main() {
    ...
    if (input.command === "help") {
        ...
        if (!input.id || !input.invite || !input.amount) {
        ...
        const check = new Client({id: input.id, invite: input.invite, name: "BlCATRU"});
        check.on("roomConnected", data => {
            ...
            for (let i = 0; i < input.amount; i++) {
                ...
                const bot = new Client({id: input.id, invite: input.invite, name: `bicatru${i+1}`, epilepsy: input.epilepsy})
                bots.push(bot);
                bot.connect()
            }
        });
        ...
        if (!input.id || !input.invite || !input.amount|| !input.msg) {
            console.log(chalk.red("Please enter all the required information"));
            return main();
        }
        console.log(chalk.magenta("Press Ctrl+C to stop"));
        ...
        check.on("roomConnected", data => {
            ...
            for (let i = 0; i < input.amount; i++) {
                console.log(chalk.green(`Spawning bot ${i+1}/${input.amount}`));
                const bot = new Client({id: input.id, invite: input.invite, name: `bicatru${i+1}`})
                bots.push(bot);
                bot.on("roomConnected", _ =>{bot.report();})
        ...
        try {
            ...
            if(cfg.mode === "spam"){
                ...
                console.log(chalk.magenta("Press Ctrl+C to stop"));
                console.log(chalk.green("Verifying server..."));
                const check = new Client({id: cfg.id, invite: cfg.invite, name: "BlCATRU"});
                check.on("roomConnected", data => {
                    console.log(chalk.green("Server verified!"));
                    check.disconnect();

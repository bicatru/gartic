...
class Client extends EventEmitter {
    ...
    connect() {
        ...
        this.ws.on("open", () => {
            ...
            this.emit("connected");
            this.ws.send(`42[3,{"v":20000,"nick":"${this.name}","avatar":0,"sala":"${this.invite}"}]`);
        });
    ...
    report() {
    if(this.destroyed) return;
    this.ws.send(`42[35,${this.room[2]}]`);
    return this;
}
    sendChat(msg) {
        if(this.destroyed) return;
    ...
    kick() {
        if(this.destroyed) return;
        this.ws.send(`42[45,${this.room[2]},["aade43824-adde-4ff2-b2ec-091409c4c10c",true]`);
        return this;
    }
    disconnect() {
        try {
